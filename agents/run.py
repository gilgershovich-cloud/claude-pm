...
Runs CEO + 8 VP agents in parallel threads.
...
Hierarchy:
  CEO (daily digest + lifecycle decisions)
    ...
    └── VP Investor Relations  (ClinicOS seed raise, pipeline, follow-ups)
"""
import threading, logging, os, sys, time
sys.path.insert(0, os.path.dirname(__file__))

import ceo, vp_engineering, vp_marketing, vp_projects, vp_sales, vp_finance, vp_it, vp_hr, vp_investor_relations
...
if __name__ == "__main__":
    logger.info("🏢 AI Company OS — starting (CEO + 8 VPs)")
    ...
    threads = [
        ...
        threading.Thread(target=_loop, args=(vp_investor_relations,  "IR_INTERVAL_SECONDS",        14400, "VP Investor Relations"),  daemon=True),
    ]
    ...
    logger.info("🏢 All 9 agents running. Company is autonomous.")
